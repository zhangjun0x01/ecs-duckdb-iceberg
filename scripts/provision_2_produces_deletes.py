...
spark = (
    ...
    .config("spark.sql.catalog.demo.warehouse", "data-with-deletes")
...
spark.sql(
    """
        CREATE OR REPLACE TABLE demo.pyspark_iceberg_table
        TBLPROPERTIES (
            'format-version'='2',
            'write.update.mode'='merge-on-read'
        )
...
# # spark.sql("update default.lineitem set l_orderkey=5 where l_partkey % 1000000 = 0")
# spark.sql("""
#     delete from default.lineitem where
#     (l_orderkey = 35 and l_partkey = 119917) or
#     (l_orderkey=5998726 and l_partkey=104203);
# """)

spark.sql("""
update demo.pyspark_iceberg_table
...
spark.sql("""
insert into demo.pyspark_iceberg_table
select * FROM demo.pyspark_iceberg_table
...
spark.sql("""
delete
from demo.pyspark_iceberg_table
...
spark.sql("""
ALTER TABLE demo.pyspark_iceberg_table
